...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const { email, name, password, cgpa, phone } = req.body;
    ...
    const { rows } = await query(
      ...
      [email, name, passwordHash, cgpa, phone ?? null]
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { rows } = await query(
      'SELECT id, email, name, cgpa, phone, password_hash FROM students WHERE email = $1',
      [email]
    );
    const student = rows[0];
